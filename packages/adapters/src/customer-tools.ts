...
export const customerTools: ConnectorTool[] = [
  ...
  },
  {
    ...
    readOnly: true,
    description:
      "Inspect your customer behavior, connected messaging channels, receive errors, and available runtime/account connections. Each connection's customerActions lists actions shared by its current account policy, not proof of provider availability. Workflow steps must be shared there; if missing, direct the owner to turn off Internal for the named action in connection settings before retrying practice. customerKnowledge reports the effective approved learning documents plus attached-library and legacy-search configuration. behavior.knowledge is only a legacy search connection; null does not mean approved knowledge is missing. Attached does not prove source readiness or freshness. Model connections include this Space's selected modelId and isDefault preferences; modelOverride identifies this bot's explicit choice. A connection label names the provider, not a model. Use the selected model ID with its credential. Customer runtime details are internal; speak to the user as one staff identity.",
    inputSchema: { type: "object", properties: {} },
  },
  {
