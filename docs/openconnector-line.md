...
The reusable offline HTTP fixture in `packages/adapters/src/open-connector-test-fixture.ts` verifies provisioning, encrypted grants, account-specific execution and removal. Adapter and authenticated RPC tests cover same-team use, cross-team denial, creator-only management, invalid credentials, sibling accounts, scoped runtime grants, and provisioning cleanup. Browser tests cover desktop and narrow web account setup and server configuration using fake credentials. A live LINE send still needs deployment credentials and an eligible test recipient.
...
Offline verification lives in `connection-channels.test.ts`, `line-webhook.test.ts`, the existing bridge tests, and the LINE connector browser tests. These cover encrypted setup, ownership/team boundaries, signature rejection, empty verification, account-scoped routing, disable checks, and the web setup flow at desktop and phone widths. Live verification still requires the account owner to supply the channel secret locally and send a message in LINE.
