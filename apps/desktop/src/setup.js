(() => {
  ...
  const PHASE_LABELS = {
    ...
    pulling: "Downloading Deskazo…",
    starting: "Starting Deskazo…",
    "waiting-healthy": "Almost ready…",
    ready: "Deskazo is ready.",
  };
  ...
  async function check() {
    ...
    try {
      ...
      if (result.ok) {
        ...
        setStatus(`Deskazo answered at ${result.url}.`, "ok");
      } else {
  ...
  async function init() {
    ...
    try {
      ...
      setStatus("Setup could not start. Quit Deskazo and try again.", "error");
      setBusy(true);
    }
  }
