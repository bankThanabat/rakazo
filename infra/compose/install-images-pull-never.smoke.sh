...
[[ "$offline_out" == *"Deskazo is starting"* ]] || fail "--offline did not start"
...
[[ "$old_out" != *"Deskazo setup failed:"* ]] || fail "old Compose --offline should not hard-fail: $old_out"
[[ "$old_out" == *"Deskazo is starting"* ]] || fail "old Compose --offline should continue: $old_out"
