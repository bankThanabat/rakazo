...
describe("agent content negotiation", () => {
  ...
  it("maps canonical and trailing-slash page paths to Markdown documents", () => {
    expect(getMarkdownDocument("/")).toContain("# Deskazo");
    expect(getMarkdownDocument("/about/")).toContain("# About Deskazo");
  ...
  it("publishes specific when-to-use instructions for agents", () => {
    ...
    expect(AGENT_INSTRUCTIONS).toContain("## When to use Deskazo");
    expect(AGENT_INSTRUCTIONS).toContain("## How an agent should use Deskazo");
  ...
  it("returns cache-safe Markdown responses and omits bodies for HEAD", async () => {
    const response = markdownResponse("# Deskazo\n");
    ...
    await expect(response.text()).resolves.toBe("# Deskazo\n");

    const headResponse = markdownResponse("# Deskazo\n", "HEAD", 404);
