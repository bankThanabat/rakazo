...
async function packagedExecutable() {
  ...
  const candidates = process.platform === "darwin" ? await findNamed(out, "Deskazo.app") : [];
  if (process.platform === "darwin" && candidates[0]) {
    return path.join(candidates[0], "Contents/MacOS/Deskazo");
  }
...
async function measureBundles() {
  ...
  const applications = await findNamed(path.join(desktopRoot, "out"), "Deskazo.app");
...
function renderMarkdown(report: PerformanceReport) {
  ...
  return `# Deskazo desktop performance — ${report.label}
