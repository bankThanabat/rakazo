...
export const customerTools: ConnectorTool[] = [
  ...
  },
  {
    ...
    description:
      "Publish a customer agent revision to stock Langflow. runtime selects {credential, baseUrl} for Langflow with base URL ending /api/v1. credential is a saved bot secret name bound to that origin with header auth x-api-key; use list_secrets and request_secret, never model settings, to configure service credentials. modelCredentialId and modelId select the saved model connection used through Rakazo’s model bridge. An attached Documents library provides customer knowledge from sources with Internal off. Leave knowledge and knowledgeFilterId unset when a library is attached. For legacy filters only, knowledge selects {credential, baseUrl} for OpenRAG ending /v1, and knowledgeFilterId selects explicit published sources. Inspect any connected OpenConnector action schemas to build general workflows; do not hardcode a business provider. Never ask the user for a flow ID or to open a flow editor. Use the existing connection/approval UI. Only public business instructions belong here; never copy private staff instructions or memory. The knowledge filter must select explicit published data sources; omitted/null means no legacy knowledge access. Named workflows bind connectionId, input schema, and ordered steps. Steps use exact $input.field, $steps.step.field, $customerId and $threadId placeholders. Customer-specific reads and all writes need a read checking an ownership field equals $customerId; bind later record IDs to that checked result. Use audience public only for deliberately public data. Before enabling writes, verify current eligibility, record and amount limits, and rejection of the same operation in a later customer turn. Per-turn deduplication is not business idempotency; otherwise grant reads and hand writes to staff. The customer agent can request_human; failed or unsupported actions should hand off. Requires owner approval.",
