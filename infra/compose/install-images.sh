...
case "$DOWNLOAD_BASE" in
  https://*) ;;
  *)
    echo "Deskazo setup failed: RAKAZO_DOWNLOAD_BASE must use https." >&2
    exit 1
    ;;
esac
...
if [[ "$prepare_only" == true ]]; then
  echo "Deskazo files are ready. Edit .env, then run: bash install-images.sh"
  exit 0
fi
...
echo "Deskazo is starting at http://127.0.0.1:5173"
