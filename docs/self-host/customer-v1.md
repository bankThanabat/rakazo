...
The script builds verified sources and starts separate Compose projects on a shared service network. It defaults the OpenRAG administration UI to port 3001 to avoid the connector's port 3000. The OpenRAG overlay enables RBAC and its public API router and uses one worker. `API_URL` remains the public webhook origin; the overlay supplies `API_INTERNAL_URL` for scoped customer-tool callbacks. Configure OpenRAG at `http://rakazo-support-rag:8000/v1` and OpenConnector at `http://rakazo-support-connector:3000` when Rakazo runs in this stack. Host development needs host-reachable addresses instead. Configure the embedding and chat model through the existing OpenRAG connection settings; no particular hosted model vendor is required.
