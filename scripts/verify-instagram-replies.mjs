#!/usr/bin/env node
// Apply the saved Instagram patch to a disposable pinned checkout and test its real handlers.
...
const patch = join(root, "infra/open-connector-patches/instagram-comment-replies.patch");
const dependencies = await realpath(join(source, "node_modules"));
const directory = await mkdtemp(join(tmpdir(), "deskazo-instagram-replies-"));
...
try {
  ...
  await run("git", ["add", "--intent-to-add", "src/providers/instagram/comment-replies.test.ts"]);
  ...
  await run(process.execPath, [
    join(root, "scripts/verify-openconnector-read-effects.mjs"),
    checkout,
  ]);
  console.log(
    JSON.stringify(
      {
        ...
        verification: [
          ...
          "offline reply handler tests",
          "audited read effects",
